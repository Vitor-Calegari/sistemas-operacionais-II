...
int main() {
    ...
    if (pid == 0) {  
        ...
        for (int j = 0; j < num_messages_per_comm; j++){
            ...
            // Calcula a latência
            auto t_recv_us = duration_cast<microseconds>(t_recv.time_since_epoch()).count();
        ...
        double avg_latency_us = (msg_count > 0 ? static_cast<double>(total_latency_us) / msg_count : 0);
...
// g++ -std=c++20 tests/src/api_test.cc -Iinclude -o api_test src/engine.cc src/ethernet.cc src/message.cc src/utils.cc
// sudo ./api_test
