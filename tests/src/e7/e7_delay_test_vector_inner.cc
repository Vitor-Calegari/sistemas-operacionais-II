...
int main() {
  ...
  pthread_mutex_t *shared_mutex = (pthread_mutex_t *)mmap(
      ...
      MAP_SHARED | MAP_ANONYMOUS, -1, 0);
  int64_t *max_shared =
      (int64_t *)mmap(NULL, sizeof(int64_t), PROT_READ | PROT_WRITE,
                      MAP_SHARED | MAP_ANONYMOUS, -1, 0);
  int64_t *min_shared =
      (int64_t *)mmap(NULL, sizeof(int64_t), PROT_READ | PROT_WRITE,
                      MAP_SHARED | MAP_ANONYMOUS, -1, 0);
  ...
  *min_socket = INT64_MAX;
  ...
  if (getpid() != parent_pid) {
    ...
    for (const auto& pair : socket_points) {
      socket_delta += pair.second;
      socket_counter++;
    }
    max_sock_int = car.get_max_socket_delay();
    min_sock_int = car.get_min_socket_delay();

    shared_mem_points = car.get_shared_delays();
    for (const auto& pair : shared_mem_points) {
      shared_mem_delta += pair.second;
      shared_mem_counter++;
    }
    max_shared_int = car.get_max_shared_delay();
    min_shared_int = car.get_min_shared_delay();
